...
public class Example02 {
	...
	public static void main(String[] args) {
		...
		//       - 필드 salary는 월급여액을 저장하며, int형으로 초기값으로 1,000,000 저장
		//       - 메소드 getAnnualGross()는 연봉을 계산해 반환하는 메소드로
		//       - 연봉은 열두달치 월급여액에 한달치 월급여액 * 500%를 더한 값으로 한다
	    //       - 기본 생성자에서 필드 salary의 초기값을 사용하며,
		...
		// 기본 생성자는 없으며,
		// 생성자를 통해 학생이름과 학급이름을 저장한다.
		// 메서드는 다음과 같이 구성된다.
		// 1) 각 필드별 getter와 setter
		// 2) 메소드 getTotal()은 모든 과목의 점수를 더한 총합을 반환한다.
		// 3) 메소드 getAverage()는 모든 과목의 점수 평균을 반환한다.
		
		StudentScore studentHong = new StudentScore("홍길동", 1);
		studentHong.setKorean(100);
		studentHong.setEnglish(60);
		studentHong.setMath(76);
		
		System.out.println("학생 " + studentHong.getName() + "의 총점은 "+ studentHong.getTotal() + "이고, "
				           + "평균은 " + studentHong.getAverage() + "입니다.");
        ...
        - 메소드는 다음과 같이 구성된다.
           ...
           3) 메소드 withdraw은 외부에서 매개변수로 받은 값을 인출한다.       
    */
   
   Account account = new Account("이효림");
